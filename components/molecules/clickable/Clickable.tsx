...
type Type = "primary" | "secondary" | "alert" | "cta"
...
export const Clickable: React.FC<ClickableProps> = ({
  ...
  return (
    ...
    >
      {loading ? (
        <span className="w-4 h-4 text-lightBlue-400">
          <Spinner />
        </span>
      ) : (
        <>
...
const colors = (type: Type): string => {
  ...
  const options: Record<Type, string> = {
    ...
    cta: "from-blue-700 to-lightBlue-500 text-white inline-flex hover:shadow-glow justify-center w-full text-sm text-center border border-blue-500 rounded-sm cursor-pointer hover:border-0 bg-gradient-to-tr  focus-within:bg-blue-800 focus-within:border-blue-800 focus-within:text-white sm:text-base md:text-lg",
  }
